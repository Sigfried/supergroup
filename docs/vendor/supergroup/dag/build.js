...
export function buildDag(items) {
    const ctx = { totalRecords: 0 };
    const byId = new Map();
    for (const it of items) {
        if (byId.has(it.id))
            throw new Error(`duplicate id: ${it.id}`);
        byId.set(it.id, new SGNode({ id: it.id, key: it.id, label: it.name ?? it.id, ctx }));
    }
    const backedges = [];
    const childIds = new Map(); // candidate edges, item order
    ...
    for (const it of items) {
        for (const pid of it.parentIds ?? []) {
            ...
            if (!p)
                continue; // unknown parent ids ignored
            const ek = `${pid}→${it.id}`;
            if (seenEdge.has(ek))
    ...
    return new Supergroup(roots, { backedges, ctx });
}
