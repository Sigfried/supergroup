 ...
 * @param {Array|Ref<Array>} records - Records to group (can be reactive)
 * @param {String|Function|Array|Ref} dimensions - Single dimension or array for multi-level
...
export function useGrouping(records, dimensions, options = {}) {
  ...
  const groupByDimension = (recs, dim, opts) => {
    ...
    // Create group values
    const groupValues = _.map(_.toPairs(groups), ([key, groupRecs], i) => {
      const value = {
  ...
  const grouped = computed(() => {
    const recs = toValue(recordsRef);
    const dims = toValue(dimensionsRef);
    const opts = toValue(optionsRef);
